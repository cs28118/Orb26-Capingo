...
function handleGeminiError(err, res) {
  if (err.status === 400) {
    return res.status(400).json({ error: 'Invalid request. Check your payload limits.' });
  }
  if (err.status === 401 || err.status === 403) {
    return res.status(403).json({ error: 'Gemini API Key is invalid or unauthorized.' });
  }
  return res.status(500).json({ error: err.message || 'Unexpected server error communicating with AI' });
}
...
app.get('/api/health', async (req, res) => {
  if (!GEMINI_API_KEY) {
    return res.status(503).json({ ok: false, ai: false, error: "Missing API Key" });
  }
  res.json({ ok: true, ai: true, model: GEMINI_MODEL });
});
...
app.post('/api/summarize', async (req, res) => {
  ...
  try {
    const summary = await callGemini(geminiMessages);
